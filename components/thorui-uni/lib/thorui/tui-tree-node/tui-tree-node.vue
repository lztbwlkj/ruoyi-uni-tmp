<template>
	<view class="tui-tree__node">
		<view class="tui-tree__label" style="width: 100%;" @tap="handleClick">
			...
			<template v-if="node.src">
				<image class="tui-tree__icon" mode="widthFix" :src="node.src"
					:class="{'tui-treeview__icon-hidden':!collapsed}">
				</image>
			</template>
			<template v-if="node.activeSrc">
				<image class="tui-tree__icon" mode="widthFix" :src="node.activeSrc"
					:class="{'tui-treeview__icon-hidden':collapsed}">
				</image>
			</template>
			<view>{{ node[keyName] }}</view>
			<view style="margin-left: auto;" v-if="node.user && node.check">
				<u-icon name="checkbox-mark" color="#4f77ed"></u-icon>
			</view>
		</view>
		<template v-if="node.children && node.children.length > 0">
			<view class="tui-tree__children" v-show="!collapsed">
				<tui-tree-node v-for="(item,index) in node.children" :key="index" :node="item" :keyName='keyName'

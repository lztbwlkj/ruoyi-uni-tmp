<template>
	<view class="tui-container">
		<view class="tui-upload-box">
			<view class="tui-image-item" :style="{width:width+'rpx',height:height+'rpx',marginRight:'20rpx'}"
				v-for="(item,index) in imageList" :key="index">

				<image :src="vuex_filepre + item.fileUrl" class="tui-item-img"
					:style="{width:width+'rpx',height:height+'rpx'}" @tap.stop="previewImage(index)" mode="aspectFill">
				</image>
				...
				<view v-if="statusArr[index]!=1" class="tui-upload-mask">
					...
					<view class="tui-tips">{{statusArr[index]==2?'上传中...':'上传失败'}}</view>
						...
						success: function(res) {
							if (res.statusCode == 200) {
								...
								if (d) {
									...
									d && (_this.imageList[index] = d)
									_this.$set(_this.statusArr, index, d.fileUrl ? "1" : "3")
								} else {
									// 上传失败
